...
export function parseAttackDescription(description: string): ParsedAttack {
  const result: ParsedAttack = { isAttack: false };
  ...
  result.attackType = attackMatch[1].charAt(0).toUpperCase() + attackMatch[1].slice(1).toLowerCase() as 'Melee' | 'Ranged';
 ...
 */
export function parseSavingThrows(savingThrows: string): Record<string, string> {
  const result: Record<string, string> = {};
 ...
 */
export function parseSkills(skills: string): Record<string, string> {
  const result: Record<string, string> = {};
